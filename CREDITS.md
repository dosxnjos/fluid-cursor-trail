...
| **Michael Brusegard** | [WebGL-Fluid-Enhanced](https://github.com/michaelbrusegard/WebGL-Fluid-Enhanced) | Fork em classe com `start()/stop()` e config camelCase. A lib vem **embutida inline** em `testbed/index.html`. | MIT |
...
SSR/cleanup no componente React e a bancada de tuning (`testbed/`). O motor da
simulação é inteiramente dos autores acima.
